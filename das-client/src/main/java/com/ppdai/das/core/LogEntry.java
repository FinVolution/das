...
public class LogEntry {
	...
	private static String execludedPackageSpace = "com.ppdai.das.";
	...
	private DasDiagnose statementDiagnose;
	...
	public LogEntry(){
        ...
        StackTraceElement ste = currentCaller.get();
        if(ste != null){
        	this.stackTraceElement = ste;
            dao = ste.getClassName();
	...
	public void setEvent(EventEnum event) {
		...
		if(this.event == EventEnum.CALL || 
				this.event == EventEnum.BATCH_CALL) {
			commandType = "SP";
		} else if(this.event == EventEnum.QUERY) {
			commandType = "Query";
		} else {
			commandType = "Execute";
		}
    ...
    public int getSqlSize() {
		int size = 0;
		if (this.event == EventEnum.QUERY
				|| this.event == EventEnum.UPDATE_SIMPLE
				|| this.event == EventEnum.UPDATE_KH
				|| this.event == EventEnum.BATCH_UPDATE_PARAM) {
			size = null != this.sqls && this.sqls.length > 0 ? this.sqls[0].length() : 0;
		}
		if (this.event == EventEnum.BATCH_UPDATE) {
