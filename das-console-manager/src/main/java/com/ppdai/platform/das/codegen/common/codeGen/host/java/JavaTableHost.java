...
public class JavaTableHost {
    ...
    private String pojoViewName;
    private List<String> columnDefinitions;
    private Integer field_type;


    public String getCustomTableame() {
        return customTableame;
    }

    public void setCustomTableame(String customTableame) {
        this.customTableame = customTableame;
    }
    ...
    public String getPojoViewName() {
        return pojoViewName;
    }

    public void setPojoViewName(String pojoViewName) {
        this.pojoViewName = pojoViewName;
    }
    ...
    public List<JavaParameterHost> getFields() {
        /*if (fields != null) {
            for (JavaParameterHost field : fields) {
                String name = field.getClassDisplayName();
                System.out.println(field.getJavaClass().toString());
    ...
    public Set<String> getPojoImports() {
        Set<String> imports = new TreeSet<>();
        List<JavaParameterHost> allTypes = new ArrayList<>(fields);
        List<JavaParameterHost> list = allTypes.stream().filter(field -> DataFieldTypeEnum.SQL_DATE.getDetail().equals(field.getJavaClass().getName())).collect(Collectors.toList());
        for (JavaParameterHost field : allTypes) {
            Class<?> clazz = field.getJavaClass();
            if (byte[].class.equals(clazz)) {
                continue;
            ...
            if (null != field_type && field_type == DataFieldTypeEnum.UTIL_DATE.getType() && DataFieldTypeEnum.SQL_TIMESTAMP.getDetail().equals(clazz.getName())) {
                if (CollectionUtils.isNotEmpty(list)) {
                    continue;
                }
                imports.add(DataFieldTypeEnum.UTIL_DATE.getDetail());
            } else {
                imports.add(clazz.getName());
            }
        }
