...
export default class ApolloSaecCkeckResult extends Component {
    ...
    createShow = list => {
        if (list) {
            return list && list.map((v, i) => {
                const apollo = v.apollo, das = v.das, saec = v.saec
                return <Row key={i}>
                    <Col sm={6} className='col-border'>
                        <Alert message={apollo.key} showIcon type={apollo.type}/>
                    </Col>
                    <Col sm={6} className='col-border'>
                        <Alert message={das.value} showIcon type={das.type}/>
                    </Col>
                    <Col sm={6} className='col-border'>
                        <Alert message={apollo.value} showIcon type={apollo.type}/>
                    </Col>
                    <Col sm={6} className='col-border'>
                        <Alert message={saec.value} showIcon type={saec.type}/>
                    </Col>
                </Row>
            })
        }
    ...
    getMsg = checkData => {
        ...
        switch (checkData.code) {
            ...
            case 300:
                return <Alert
                    message={'数据校验通过,但' + UserEnv.getSecurityCenterName() + '或' + UserEnv.getConfigCenterName() + '数据与DAS出现多余属性值！！！'}
                    type='warning' showIcon
                    description={description}/>
            case 500:
                return <Alert
                    message={'数据校验不通过！请尝试同步到' + UserEnv.getSecurityCenterName() + '或' + UserEnv.getConfigCenterName() + '数据'}
                    type='error' description={description} showIcon/>
        }
    }
    ...
    render() {
        ...
        const columnTitle = checkData.item.columnTitle ? checkData.item.columnTitle : ['das', '配置中心', '安全服务']
        return <div className='apolloCkeckResult'>
            <Row>
                <Col sm={24} className='col-border' style={{paddingBottom: '15px'}}>
                    {this.getMsg(checkData)}
                </Col>
            </Row>
            <div style={{display: showDetail ? 'block' : 'none'}}>
                ...
                <Row>
                    ...
                    <Col sm={6} className='col-border'>
                        <Alert message={columnTitle[2]} type='info'/>
                    </Col>
                </Row>
